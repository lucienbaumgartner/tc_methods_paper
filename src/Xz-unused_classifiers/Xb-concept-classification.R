## ---------------------------
##
## Script name: xx-concept-classification.R
##
## Project: Tracing Thick Concepts
##
## Purpose of script: Classifier for concept classes
##
...
metric <- "Accuracy"
## Draw sample to test models
set.seed(7826)
df <- sample_n(df, 10000)
## Make test train split
set.seed(7826)
validation_index <- createDataPartition(df$CLASS, p=0.80, list=FALSE)
...
## a) Linear algorithms
set.seed(7826)
fit.lda <- train(CLASS~., data=trainSet, method="lda", metric=metric, trControl=control)
## b) Nonlinear algorithms
## CART
set.seed(7826)
fit.cart <- train(CLASS~., data=trainSet, method="rpart", metric=metric, trControl=control)
## kNN
set.seed(7826)
fit.knn <- train(CLASS~., data=trainSet, method="knn", metric=metric, trControl=control)
## c) Advanced algorithms
## SVM
set.seed(7826)
fit.svm <- train(CLASS~., data=trainSet, method="svmRadial", metric=metric, trControl=control)
## Random Forest
set.seed(7826)
fit.rf <- train(CLASS~., data=trainSet, method="rf", metric=metric, trControl=control)
...
confusionMatrix(predictions, validation$CLASS, mode = "prec_recall")
...
save(fit.cart, fit.knn, fit.lda, fit.rf, fit.svm, trainSet, validation, predictions, 
     file = paste0('../output/04-classification-models/target_concept_class_', time_index, '.Rdata'))
